...
app.post('/update-approval-status2', (req, res) => {
  const { name, amount, approval_status } = req.body;

  // Perform an SQL update operation to update the approval status
  const sql = 'UPDATE maturity_date_data SET approval_status = ? WHERE name = ? AND amount = ?';

  // Execute the SQL query
  db.query(sql, [approval_status, name, amount], (err, result) => {
    if (err) {
      console.error('Error updating approval status:', err);
      return res.status(500).json({ error: 'Internal server error' });
    }
...
app.get('/get-maturity-data', (req, res) => {
  ...
  console.log('hi');
  const query = 'SELECT amount, interest, duration FROM maturity_date';
...
app.post('/update-solution', (req, res) => {
  const { key, solution, subject, name } = req.body;

  // Update the solution in the database
  const sql = 'UPDATE report_issue SET solution = ?, subject = ?, Name = ?';
  db.query(sql, [solution, subject, name, key], (err, result) => {
    if (err) {
